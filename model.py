...
class RNNModel(nn.Module):
    ...
    def __init__(self, rnn_type, ntoken, ninp, nhid, nlayers, dropout=0.5, dropouth=0.5, dropouti=0.5, dropoute=0.1, wdrop=0, tie_weights=False):
        ...
        self.linear_out = nn.Linear(400 * 2, 400, bias=False)
    ...
    def forward(self, input, hidden, return_h=False):
        ...
        for j in range(raw_output.size(1)): 
            ...
            attention_scores = torch.bmm(result_sub, raw_output.transpose(1, 2).contiguous().cuda())

            attention_scores = attention_scores.view(batch_size * output_len, query_len)
            attention_weights = self.softmax(attention_scores)
            attention_weights = attention_weights.view(batch_size, output_len, query_len)

            mix = torch.bmm(attention_weights, raw_output.cuda())

            combined = torch.cat((mix, result_sub), dim=2)
            combined = combined.view(batch_size * output_len, 2 * 400)

            output = self.linear_out(combined).view(batch_size, output_len, 400)
